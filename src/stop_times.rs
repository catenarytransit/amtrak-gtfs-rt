...
pub async fn query_all_trips_simultaniously(train_numbers: &Vec<(String,NaiveDate)>) -> HashMap<(String, NaiveDate), RootTripData> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!("Error fetching data for train number {:?}: {}", train_number, e);
            }
        }
    }
...
pub async fn get_stop_times(train_number: &str, starting_date: &NaiveDate, client: &reqwest::Client) -> Result<RootTripData, Box<dyn std::error::Error>> {
 let url = format!("https://amtraktime.catenarymaps.org/amtrakstatus?trainnum={}&starting_date={}", train_number, starting_date.format("%Y-%m-%d"));
 ...
 headers.insert("Accept", "application/json, text/plain, */*".parse()?);
    ...
    if let Err(deserialized) = &deserialized {
        eprintln!("Error deserializing response for train number {}: {}", train_number, deserialized);
